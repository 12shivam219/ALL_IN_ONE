...
class OneDriveManager(CloudStorageManager):
    """OneDrive integration - attempts to resolve path from Windows registry or environment"""
    ...
    def _get_onedrive_path(self) -> Optional[str]:
        # Try finding OneDrive path from environment or settings
        path = os.getenv("ONEDRIVE_FOLDER_PATH") or settings.ONEDRIVE_FOLDER_PATH
    ...
    def list_files(self, folder_name: str = "Resumes") -> List[Dict]:
        if not self.onedrive_path:
            logger.error("OneDrive path not resolved on this machine")
            return []
...
class GoogleDriveManager(CloudStorageManager):
    """Google Drive integration using credentials.json and token.pickle"""
    ...
    def list_files(self, folder_path: str = "/Resumes") -> List[Dict]:
        ...
        try:
            ...
            for entry in result.entries:
                ...
                if isinstance(entry, FileMetadata) and entry.name.lower().endswith(('.docx', '.pdf', '.doc')):
                    files.append({
                        'name': entry.name,
